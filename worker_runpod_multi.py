...
def image_to_3d(image_paths: List[str], seed: int = 0, randomize_seed: bool = True,
                ss_guidance_strength: float = 7.5, ss_sampling_steps: int = 12,
                slat_guidance_strength: float = 3.0, slat_sampling_steps: int = 12, preprocess_image: bool = True) -> Tuple[dict, str]:
...
def generate(input):
    ...
    for i, image in enumerate(images):
        image_path = download_file(url=image['url'], save_dir='/content', file_name=f'input_image_{i}')
    ...
    preprocess_image = values['preprocess_image']

    state, video_path = image_to_3d(image_paths=input_images, 
                                    seed=seed, 
    ...
    result = ["/content/trellis-tost.mp4", ["/content/trellis-tost.glb"]]
    try:
